...
class YuvToRGBConverter(context: Context) {
    ...
    fun yuvToRGB(image: Image, output: Bitmap) {
        if (!::yuvBuffer.isInitialized) {
            ...
            val pixelSizeits = ImageFormat.getBitsPerPixel(ImageFormat.YUV_420_888)
            yuvBuffer = ByteBuffer.allocateDirect(pixelCount * pixelSizeits / 8)
        }
        yuvBuffer.rewind()
        ...
        if (!::inputAllocation.isInitialized) {
            val elemType = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
